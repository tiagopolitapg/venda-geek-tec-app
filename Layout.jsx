...
import { useAuth } from './lib/AuthContext'; // Importação corrigida
import { 
  LayoutDashboard, 
  Package, 
  Users, 
...
export default function Layout({ children, currentPageName }) {
  ...
  const { user, logout } = useAuth(); // Usando useAuth
  const [mobileOpen, setMobileOpen] = useState(false);

  const handleLogout = () => {
    logout(); // Usando a função logout do AuthContext
  };
  ...
  // Adicionar item de Gerenciamento de Usuários se for admin
  const navigationItems = [...baseNavigationItems];
  if (user?.role === 'admin') {
    navigationItems.push({
      title: "Usuários",
      url: createPageUrl("Users"), // Rota que será criada
      icon: Users,
    });
  }
